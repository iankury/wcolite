...
let dragCurX = 0, dragCurY = 0, dragRunning = false
let lastQuery = '', backupQuery = ''
...
function sendPlaceholderPage() {
  if (!lastQuery) {
    if (mode == 'table')
      setLastQuery('mello-leitão')
    else
      setLastQuery('cryptogeobiidae')
  }
  sendFirstPage()
}
...
function displayTable() {
  ...
  for (x of data.resultList) {
    ...
    const authorship = $('<div class="data-table__authorship data-column"></div>')
...
function setTreeDrag() {
  ...
  $('#data-tree__container').on(dragOn, e => {
...
function updateHowTo() {
  ...
  if (mode == 'tree')
    ...
    s = 'Left click links to use taxon as new table query, right click to view taxon cards.'
  else
    s = 'All ancestors, as well as all children, are links.'
...
function didYouMean() {
  $('.data__popup-mean p').html(`Did you mean <span>${data.didYouMean}</span>?`)
